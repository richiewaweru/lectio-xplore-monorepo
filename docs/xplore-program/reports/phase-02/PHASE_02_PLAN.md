...
## Goal
Sharpen workspace `packages/lectio` into web-native `@lectio/learn` without breaking Builder or generated Learn documents; keep `@lectio/page` independently green.

## Inspected facts
- Package lives at `packages/lectio`, name `lectio@0.6.0`, exports `.`, `./theme.css`, `./print`, contracts, components.
...
3. Rename package to `@lectio/learn` (keep path `packages/lectio` OR move to `packages/lectio-learn` — prefer rename in place with package.json name change to minimize churn).
