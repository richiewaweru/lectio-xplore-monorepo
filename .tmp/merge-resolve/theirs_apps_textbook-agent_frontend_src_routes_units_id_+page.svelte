...
<svelte:head><title>{ws.unit ? `${ws.unit.title} Â· Units` : 'Unit Â· Lectio'}</title></svelte:head>

<div class="unit-page">
	{#if ws.loading && !ws.unit}
		<p class="loading" role="status">Loading unitâ€¦</p>
	{:else if ws.unit}
		<header class="unit-head">
			<div><a href="/units" class="back">â†� Units</a><p class="eyebrow">{ws.unit.subject} Â· {ws.unit.grade_level}</p><h1>{ws.unit.title}</h1><p>{ws.unit.destination_objective}</p></div>
			<div class="head-actions">
				{#if ws.path}<span class:approved={ws.path.status === 'approved'}>{ws.path.status === 'approved' ? 'Locked in' : 'Draft'}</span>{/if}
				<button class="secondary" type="button" disabled={ws.laneBusy('plan') || !ws.actionAllowed('plan')} onclick={() => ws.planOrReplan(Boolean(ws.path))}>{ws.laneBusy('plan') ? 'Planningâ€¦' : ws.path ? 'Replan the lessons' : 'Plan the lessons'}</button>
			</div>
		</header>
		...
		{/if}
		{#if ws.dirty}
			<p class="dirty-hint" role="status">Unsaved lesson edits â€” progress updates will not overwrite them.</p>
		{/if}
		...
		{#if !ws.path}
			<section class="empty">
				{#if ws.planningFailed}
					...
					<p>Lesson planning did not complete. Nothing else was corrupted â€” try planning again on this same unit.</p>
					<button class="primary" type="button" disabled={ws.laneBusy('plan')} onclick={() => ws.planOrReplan(false)}>{ws.laneBusy('plan') ? 'Planning your lessonsâ€¦' : 'Try planning again'}</button>
				{:else}
					...
					<button class="primary" type="button" disabled={ws.laneBusy('plan')} onclick={() => ws.planOrReplan(false)}>{ws.laneBusy('plan') ? 'Planning your lessonsâ€¦' : 'Plan the lessons'}</button>
				{/if}
			</section>
		{:else}
			...
			{#if ws.path.status !== 'approved'}
				<section class="lock-in-bar">
					...
					<div class="lock-in">
						<button class="primary" type="button" disabled={ws.laneBusy('approve') || !ws.canLockIn || !ws.actionAllowed('approve')} onclick={() => ws.approvePath()}>{ws.laneBusy('approve') ? 'Locking it inâ€¦' : 'Looks good â€” lock it in'}</button>
					</div>
				</section>
			{/if}
			...
			<div class="workspace">
				<aside class="path-list" aria-label="Your lessons">
					...
					<ol>{#each ws.path.lessons as lesson, index (lesson.id)}<li class:active={lesson.id === ws.selectedId} class:skipped={lesson.skipped}><button type="button" onclick={() => ws.selectLesson(lesson)}><span>{index + 1}</span><span><strong>{lesson.title}</strong>{#if ws.dependencySentences(lesson).length}<small>{ws.dependencySentences(lesson).join(' Â· ')}</small>{:else if lesson.pack_id}<small>prepared</small>{/if}</span></button></li>{/each}</ol>
				</aside>
				...
				{#if ws.selected}
					<main class="inspector">
						...
						<form class="editor" onsubmit={(e) => ws.saveLesson(e)}>
							...
							<button class="secondary" type="submit" disabled={ws.laneBusy('save') || !ws.actionAllowed('save')}>{ws.laneBusy('save') ? 'Savingâ€¦' : 'Save lesson changes'}</button>
						</form>

						<section class="dependencies"><div><p class="eyebrow">Before this lesson</p><h3>What earlier lessons it requires</h3></div>{#if ws.dependencySentences(ws.selected).length}<ul>{#each ws.dependencySentences(ws.selected) as sentence}<li>{sentence}</li>{/each}</ul>{:else}<p>Nothing â€” this can be the starting point.</p>{/if}</section>
						...
						{#if debugMode}
							<section class="shape">
								...
								{:else}
									<p>Loading this lesson's shapeâ€¦</p>
								{/if}
							</section>
						{/if}
						...
						<section class="prepare">
								...
								{#if ws.runStatus}
									<p class="hint" role="status">
										...
										{#if ws.runStatus.completed_count != null && ws.runStatus.total_count != null}
											Â· {ws.runStatus.completed_count}/{ws.runStatus.total_count}
										{/if}
									</p>
								{/if}
							</div>
							{#if ws.preparation?.stale && ws.preparation?.can_regenerate}
								<form class="regenerate" onsubmit={(event) => { event.preventDefault(); void ws.regenerate(); }}>
									...
									<button class="primary" type="submit" disabled={ws.laneBusy('regenerate') || ws.regenerationReason.trim().length < 3 || !ws.actionAllowed('regenerate')}>{ws.laneBusy('regenerate') ? 'Making it againâ€¦' : 'Make it again'}</button>
								</form>
							{:else if ws.canStartFresh}
								<form class="regenerate" onsubmit={(event) => { event.preventDefault(); void ws.regenerate(); }}>
									...
									<button class="primary" type="submit" disabled={ws.laneBusy('regenerate') || ws.regenerationReason.trim().length < 3}>{ws.laneBusy('regenerate') ? 'Starting freshâ€¦' : 'Start fresh'}</button>
								</form>
							{:else if ws.preparation?.generation_id || (ws.preparation?.realizations?.length ?? 0) > 0}
								...
								<div class="ready-actions">
									...
									{:else if !pathHasRealization(prep, 'print')}
										<button class="primary" type="button" disabled={!ws.canGeneratePrint() || ws.laneBusy('print')} onclick={() => ws.prepare('print')}>{ws.laneBusy('print') ? 'Generating Printâ€¦' : generatePathLabel('print')}</button>
									{/if}
									...
									{:else if !pathHasRealization(prep, 'learn')}
										<button class="secondary" type="button" disabled={!ws.canGenerateLearn() || ws.laneBusy('learn')} onclick={() => ws.prepare('learn')}>{ws.laneBusy('learn') ? 'Generating Learnâ€¦' : generatePathLabel('learn')}</button>
									{/if}
									{#if prep?.realizations?.some((row) => row.status === 'read_only')}
										<p class="hint">A legacy output is read-only â€” generate an explicit Print or Learn realization from the Teaching Plan.</p>
									{/if}
			...
			<section class="chat-edit" aria-label="Edit your lessons by chat">
				...
				{#if ws.chatUnavailable}
					<p class="chat-disabled">Editing lessons by chat isn't available yet â€” use the lesson tools above for now.</p>
				{:else}
					<form onsubmit={(e) => ws.sendChatEdit(e)}>
						...
						<button class="primary" type="submit" disabled={ws.laneBusy('chat') || ws.chatMessage.trim().length < 2}>{ws.laneBusy('chat') ? 'Updatingâ€¦' : 'Send'}</button>
					</form>
			...
			{:else if ws.activeView === 'schedule'}
				...
				{:else if !ws.tabError}
					<p class="loading" role="status">Loading scheduleâ€¦</p>
				{/if}
			...
			{:else if ws.activeView === 'resources'}
				...
				{:else if !ws.tabError}
					<p class="loading" role="status">Loading resourcesâ€¦</p>
				{/if}
			{:else if ws.activeView === 'history'}
				<section class="history-panel">
					...
					{#if ws.historyLoaded}
						...
						{#if ws.viewedVersion}<div class="history-preview"><div><strong>v{ws.viewedVersion.version}</strong><span>{ws.viewedVersion.status} Â· {ws.viewedVersion.lessons.length} lessons</span></div><ol>{#each ws.viewedVersion.lessons as lesson}<li>{lesson.title}</li>{/each}</ol><button type="button" class="text-button" onclick={() => (ws.viewedVersion = null)}>Close preview</button></div>{/if}
					{:else if !ws.tabError}
						<p class="loading" role="status">Loading historyâ€¦</p>
					{/if}
				</section>
			{/if}
		{/if}
