		...
		await act('prepare', async () => {
			...
			if (existingGenerationId) {
				try {
					...
					// Teaching not approved yet â€” open existing prep in Studio for review
			...
			// Explicit path choice: Print â†’ studio (queue/poll native worker);
			// Learn â†’ studio teaching review with path=learn. The print viewer
...
<svelte:head><title>{unit ? `${unit.title} Â· Units` : 'Unit Â· Lectio'}</title></svelte:head>
...
<div class="unit-page">
	...
	{:else if unit}
		<header class="unit-head">
			<div><a href="/units" class="back">â†� Units</a><p class="eyebrow">{unit.subject} Â· {unit.grade_level}</p><h1>{unit.title}</h1><p>{unit.destination_objective}</p></div>
			<div class="head-actions">
				{#if path}<span class:approved={path.status === 'approved'}>{path.status === 'approved' ? 'Locked in' : 'Draft'}</span>{/if}
				<button class="secondary" type="button" disabled={busy !== null} onclick={() => planOrReplan(Boolean(path))}>{busy === 'replan' || busy === 'plan' ? 'Planningâ€¦' : path ? 'Replan the lessons' : 'Plan the lessons'}</button>
			</div>
		</header>
		...
		{#if !path}
			<section class="empty">
				{#if planningFailed}
					...
					<button class="primary" type="button" disabled={busy !== null} onclick={() => planOrReplan(false)}>{busy === 'plan' ? 'Planning your lessonsâ€¦' : 'Try planning again'}</button>
				{:else}
					...
					<button class="primary" type="button" disabled={busy !== null} onclick={() => planOrReplan(false)}>{busy === 'plan' ? 'Planning your lessonsâ€¦' : 'Plan the lessons'}</button>
				{/if}
			</section>
		{:else}
			...
			<section class="lock-in-bar">
				...
				{#if path.status !== 'approved'}
					<div class="lock-in">
						<button class="primary" type="button" disabled={busy !== null || !canLockIn} onclick={() => act('approve', async () => { path = await approveUnitPath(unitId, path as UnitPath); unit = await getUnit(unitId); }, false)}>{busy === 'approve' ? 'Locking it inâ€¦' : 'Looks good â€” lock it in'}</button>
					</div>
				{/if}
			</section>
			...
			<div class="workspace">
				<aside class="path-list" aria-label="Your lessons">
					...
					<ol>{#each path.lessons as lesson, index (lesson.id)}<li class:active={lesson.id === selectedId} class:skipped={lesson.skipped}><button type="button" onclick={() => selectLesson(lesson)}><span>{index + 1}</span><span><strong>{lesson.title}</strong>{#if dependencySentences(lesson).length}<small>{dependencySentences(lesson).join(' Â· ')}</small>{:else if lesson.pack_id}<small>prepared</small>{/if}</span></button></li>{/each}</ol>
				</aside>
				...
				{#if selected}
					<main class="inspector">
						...
						<form class="editor" onsubmit={saveLesson}>
							...
							<button class="secondary" type="submit" disabled={busy !== null}>{busy === 'save' ? 'Savingâ€¦' : 'Save lesson changes'}</button>
						</form>

						<section class="dependencies"><div><p class="eyebrow">Before this lesson</p><h3>What earlier lessons it requires</h3></div>{#if dependencySentences(selected).length}<ul>{#each dependencySentences(selected) as sentence}<li>{sentence}</li>{/each}</ul>{:else}<p>Nothing â€” this can be the starting point.</p>{/if}</section>
						...
						<section class="prepare">
							...
							{#if preparation?.stale && preparation?.can_regenerate}
								<form class="regenerate" onsubmit={(event) => { event.preventDefault(); void regenerate(); }}>
									...
									<button class="primary" type="submit" disabled={busy !== null || regenerationReason.trim().length < 3}>{busy === 'regenerate' ? 'Making it againâ€¦' : 'Make it again'}</button>
								</form>
							{:else if canStartFresh}
								<form class="regenerate" onsubmit={(event) => { event.preventDefault(); void regenerate(); }}>
									...
									<button class="primary" type="submit" disabled={busy !== null || regenerationReason.trim().length < 3}>{busy === 'regenerate' ? 'Starting freshâ€¦' : 'Start fresh'}</button>
								</form>
							{:else if preparation?.generation_id || (preparation?.realizations?.length ?? 0) > 0}
								...
								<div class="ready-actions">
									...
									{:else if !pathHasRealization(prep, 'print')}
										<button class="primary" type="button" disabled={path.status !== 'approved' || selected.skipped || busy !== null} onclick={() => prepare('print')}>{busy === 'prepare' ? 'Generating Printâ€¦' : generatePathLabel('print')}</button>
									{/if}
									...
									{:else if !pathHasRealization(prep, 'learn')}
										<button class="secondary" type="button" disabled={path.status !== 'approved' || selected.skipped || busy !== null} onclick={() => prepare('learn')}>{busy === 'prepare' ? 'Generating Learnâ€¦' : generatePathLabel('learn')}</button>
									{/if}
			...
			<section class="chat-edit" aria-label="Edit your lessons by chat">
				...
				{:else}
					<form onsubmit={sendChatEdit}>
						...
						<button class="primary" type="submit" disabled={chatBusy || chatMessage.trim().length < 2}>{chatBusy ? 'Updatingâ€¦' : 'Send'}</button>
					</form>
			...
			{:else if activeView === 'history'}
				<section class="history-panel">
					...
					{#if historyLoaded}
						...
						{#if viewedVersion}<div class="history-preview"><div><strong>v{viewedVersion.version}</strong><span>{viewedVersion.status} Â· {viewedVersion.lessons.length} lessons</span></div><ol>{#each viewedVersion.lessons as lesson}<li>{lesson.title}</li>{/each}</ol><button type="button" class="text-button" onclick={() => (viewedVersion = null)}>Close preview</button></div>{/if}
					{:else if !tabError}
