...
async def _run_chunked_stage2_pipeline(
    *,
    ...
    try:
        ...
        # Native whole-lesson path: items â†’ teaching plan â†’ halt for teacher approval.
    ...
    *,
    ...
    for index, variant in enumerate(variants):
        ...
        await _ensure_chunked_generation_row(
            ...
            user_id=user_id,
            subject=form.subject,
            context=f"{form.topic} â€” {variant.label}",
        ...
        await persist_chunked_state(
            generation_id,
            {
                ...
                "display_title": f"{form.topic} â€” {variant.label}",
                "execution_started": False,
        ...
        quiz_sections.append(
            {
                ...
                "header": {
                    "title": (
                        "Shared diagnostic"
                        if index == 1
                        else f"Shared diagnostic Â· {index}"
                    ),
                    "subject": model.subject,
...
async def post_realize_learn(
    ...
    Cross-domain handoff goes through application orchestration (not printâ†’learn).
    """
    await _load_owned_generation(generation_id, current_user.id)
...
async def post_retry_native(
    ...
    - item_generation â†’ checkpoint item_generation (work_kind pre_worker_item_retry)
    - planning_teaching â†’ checkpoint planning_teaching (work_kind pre_worker_teaching_retry)
    - post-approval stages â†’ queued for worker reclaim
