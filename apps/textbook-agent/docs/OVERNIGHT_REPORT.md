...
- `backend/src/core/health/routes.py`: Playwright health failures with empty exception text now expose the exception representation, making detached-runtime mismatches diagnosable from readiness output.
...
warning).
Health-route tests pass (12 tests, same existing warning); the detached process
has not been restarted because its parent PID is no longer inspectable.
