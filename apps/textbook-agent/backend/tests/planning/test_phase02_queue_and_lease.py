"""Phase 02 Commit A: transitions, queue approval, leased claim contention."""
...
@pytest.mark.asyncio
async def test_two_workers_cannot_both_claim(db_session_factory) -> None:
    async with db_session_factory() as session:
    ...
    winners = [item for item in (first, second) if item == gid]
    ...
    async with db_session_factory() as session:
        ...
        assert int(state["execution"]["attempt"]) == 1


@pytest.mark.asyncio
async def test_stale_active_can_be_reclaimed(db_session_factory) -> None:
    async with db_session_factory() as session:
        ...
        state["lesson_packet"] = {"lesson": {"objective": "Learn X"}}
        state["teaching_plan"] = {"arc": "test", "sections": []}
        await repo._save(state, stage="writing_blocks")

    async with db_session_factory() as session:
        claimed = await claim_next_native_job(session, worker_id="reclaimer", lease_seconds=90)
        assert claimed == gid
        generation = await session.get(GenerationModel, gid)
        assert generation is not None
        ...
        repo = PageDocumentRepository(session, gid)
        state = await repo.load_page_generation_state()
        assert state["execution"]["worker_id"] == "reclaimer"
        assert int(state["execution"]["attempt"]) == 2
