...
def _plan(section_ids: list[str] | None = None, cards: list[dict] | None = None) -> PathStructuralPlan:
...
async def test_valid_first_attempt_makes_exactly_one_call() -> None:
    stub = AsyncMock(return_value=_plan())
...
async def test_repairs_once_with_previous_output_and_exact_errors() -> None:
    broken = _plan(section_ids=["explain", "orient", "check"])  # wrong order
    stub = AsyncMock(side_effect=[broken, _plan()])
    with patch.object(agents, "_run_structured", new=stub):
        result = await agents.run_path_structural_planner(_fixed_context())
    ...
    assert repair["previous_output"]["sections"][0]["id"] == "explain"
...
async def test_second_invalid_attempt_fails_with_structured_errors() -> None:
    broken = _plan(section_ids=["explain", "orient", "check"])
    ...
    assert stub.await_count == 2
    assert any("sections[].id" in err for err in excinfo.value.errors)


async def test_wrong_card_count_triggers_repair() -> None:
    """The 409 seen three times in the browser run."""
    stub = AsyncMock(side_effect=[_plan(cards=[]), _plan()])
    with patch.object(agents, "_run_structured", new=stub):
        result = await agents.run_path_structural_planner(_fixed_context())
...
async def test_schema_failure_extracts_pydantic_messages_and_sends_no_previous_output() -> None:
    ...
    class _RetryPrompt:
        content = [{"loc": ("sections", 0, "id"), "msg": "Field required"}]
    ...
    stub = AsyncMock(side_effect=[wrapped, _plan()])
    with patch.object(agents, "_run_structured", new=stub):
        result = await agents.run_path_structural_planner(_fixed_context())
    ...
    assert repair["validation_errors"] == ["sections.0.id: Field required"]
    assert result.sections[0].id == "orient"
