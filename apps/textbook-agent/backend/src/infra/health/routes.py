...
_HEALTH_EXTENSION_UNSET = object()

def configure_health_extensions(
    *,
    readiness_checks: Sequence[ReadinessCheck] | None | object = _HEALTH_EXTENSION_UNSET,
    image_probe_runner: ImageProbeRunner | None | object = _HEALTH_EXTENSION_UNSET,
) -> None:
    """Merge health extensions; omit a kwarg to leave the existing value unchanged."""
    global _image_probe_runner
    if readiness_checks is not _HEALTH_EXTENSION_UNSET:
        # Mutate in place: CPython 3.11+ may specialize LOAD_GLOBAL on the
        # initial empty list; rebinding the name leaves specialized loads stale.
        _additional_readiness_checks.clear()
        _additional_readiness_checks.extend(list(readiness_checks or []))
    if image_probe_runner is not _HEALTH_EXTENSION_UNSET:
        _image_probe_runner = image_probe_runner  # type: ignore[assignment]
