...
V3_TIMEOUTS: dict[str, int] = {
    ...
    # Item generation uses its own executor (and therefore does not pass
    # through run_json_agent). Keep its provider timeout explicit so it cannot
    # silently fall back to RetryPolicy's generic 120-second default.
    "item_executor": int(os.getenv("V3_TIMEOUT_ITEM_SECONDS", "90")),
