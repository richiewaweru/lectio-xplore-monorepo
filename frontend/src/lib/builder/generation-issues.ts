...
function bookletIssue(item: Record<string, unknown>, target: string): BuilderIssue {
	...
	return {
		...
		kind: category,
		target_block_id:
			typeof item.target_block_id === 'string' ? item.target_block_id : undefined,
...
export function partitionGenerationIssues(
	...
	const sectionIssues = Object.fromEntries(
		sectionIds.map((sectionId) => [sectionId, [] as BuilderIssue[]])
	);
	...
	for (const [index, value] of (pack.section_diagnostics ?? []).entries()) {
		...
		sectionIssues[sectionId].push({
			...
			kind,
			component_ref:
				typeof missingComponents[0] === 'string'
					? `${missingComponents[0]}@${sectionId}`
					: undefined,
